...
const Column = ({ col }) => {
    ...
    useEffect(() => {
        ...
        const fetchCards = async () => {
            console.log(col);
            const result = await axios.get('http://localhost:7000/columns/' + col.columnId);
            setListCard(result.data.data.cards);
        };
        fetchCards();
    }, []);
    ...
    const handleAddOk = async () => {
        const result = await axios.post('http://localhost:7000/cards/', {
    ...
    const handleEditOk = async () => {
        const result = await axios.patch(`http://localhost:7000/cards/${cardId}`, {
            columnId: col.columnId,
            cardContent: content,
        });
